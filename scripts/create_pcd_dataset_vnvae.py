...
def _resolve_npz_path(path):
    ...
    if _NPZ_BASENAME_CACHE is None:
        cache = {}
        data_root = PROJECT_ROOT / "SDF" / "data"
        if data_root.is_dir():
            for cand in data_root.rglob("*.npz"):
                cache.setdefault(cand.name, []).append(cand)
        _NPZ_BASENAME_CACHE = cache
    matches = _NPZ_BASENAME_CACHE.get(p.name, [])
    if matches:
        preferred = []
        low_name = p.name.lower()
        if "combined" in low_name:
            preferred.append("libero_merge")
        preferred.extend(["libero_repair", "libero_ch", "libero", "libero_norm", "hand"])
        lowered = [str(c).lower() for c in matches]
        for tag in preferred:
            for cand, low in zip(matches, lowered):
                if f"/{tag}/" in low:
                    return cand
...
def _asset_path_from_npz_meta(npz_path):
    ...
    try:
        ...
        if isinstance(meta, dict):
            asset_path = meta.get("asset_path", None)
...
def load_latent_object_meta(latent_ckpt):
    ...
    if isinstance(object_meta, list) and object_meta:
        return object_meta

    objects = ckpt.get("objects")
    if isinstance(objects, list) and objects:
        converted = []
        for i, obj in enumerate(objects):
            if not isinstance(obj, dict):
                continue
            npz_path = obj.get("npz_path", None)
            fit_bounds = obj.get("fit_bounds", None)
            raw_bounds = obj.get("raw_bounds", None)
            bounds = fit_bounds if fit_bounds is not None else raw_bounds
            center = obj.get("center", None)
            asset_path = _asset_path_from_npz_meta(npz_path)
            converted.append(
                {
                    "object_id": int(i),
                    "npz_path": npz_path,
        ...
        if converted:
            print(f"  Loaded VNVAE latent payload objects: {len(converted)}")
            return converted

    print(f"  Warning: latent ckpt has no object_meta/objects, skip latent mapping export: {latent_ckpt}")
    return None
...
def _choose_npz_source_item(spec, meta):
    ...
    asset_stem = canonical_name(Path(asset_path).stem) if asset_path else ""
    best_item = None
    best_score = -1e9
    for idx, item in enumerate(source_items):
        ...
        mesh_stem = canonical_name(Path(mesh_path).stem or mesh_name)
        score = -0.01 * float(idx)
        if kind == "mesh":
            score += 5.0
        elif kind in {"geom", "primitive"}:
            score += 1.0
        if asset_stem and mesh_stem:
            if mesh_stem == asset_stem:
                score += 20.0
            elif mesh_stem in asset_stem or asset_stem in mesh_stem:
                score += 10.0
        if "handle" in mesh_stem:
            score -= 4.0
        if score > best_score:
            best_score = score
            best_item = item
    return best_item if best_item is not None else source_items[0]


def _default_npz_name_for_entity(entity_name):
...
def build_npz_to_body_mapping(entity_specs, gripper_spec, latent_mapping, object_meta):
    if not object_meta:
        return {}

    meta_by_oid, target_center_for_oid = _make_target_center_lookup(object_meta)
    meta_by_npz_basename = {}
    for item in object_meta:
        if not isinstance(item, dict):
            continue
        npz_name = _basename_npz(item.get("npz_path", ""))
        if npz_name:
            meta_by_npz_basename[npz_name] = item
    out = {}

    def _entry_from_spec(entity_name, spec):
        info = latent_mapping.get(entity_name, None)
        meta = None
        oid = -1
        npz_name = ""
        if info is not None:
            ...
            meta = meta_by_oid.get(oid, None)
            npz_name = _basename_npz((meta or {}).get("npz_path", "")) or str(info.get("target_npz_name", "") or "")
        if meta is None:
            npz_name = npz_name or _default_npz_name_for_entity(entity_name)
            meta = meta_by_npz_basename.get(npz_name, None)
        ...
        if meta is not None:
            template_points = _centered_surface_points_from_npz_path(
                meta.get("npz_path", ""),
                cache={},
                target_num=int(raw_points.shape[0]),
            )
        if template_points is None and npz_name:
            template_points = _centered_surface_points_from_npz_path(
                npz_name,
                cache={},
                target_num=int(raw_points.shape[0]),
        ...
        if template_points is None:
            return {
                "trans_local": raw_points.mean(axis=0).astype(np.float32, copy=False),
                "rot_wxyz": np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32),
                "mapping_mode": "npz-missing-fallback-raw-centroid",
                "target_npz_name": npz_name,
                "object_id": oid,
                "body_name": str(spec.get("body_name", "")),
                "category": str(spec.get("category", "")),
                "part": str(spec.get("part", "")),
        ...
        category_name = str(spec.get("category", "") or "")
        part_name = str(spec.get("part", "") or "")
        body_name = str(spec.get("body_name", "") or "")
        match_kind = str((info or {}).get("match_kind", "") or "")
        cabinet_part = canonical_name(part_name)
        is_cabinet_body_center_pose = (
            canonical_name(category_name) == "wooden_cabinet"
            and cabinet_part in {"base", "cabinet_bottom", "cabinet_middle", "cabinet_top"}
        )
        if is_cabinet_body_center_pose:
            surface_center = np.asarray(
                spec.get("surface_center", raw_points.mean(axis=0, dtype=np.float32)),
                dtype=np.float32,
            ).reshape(3)
            return {
                "trans_local": surface_center.astype(np.float32, copy=False),
                "rot_wxyz": np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32),
                "mapping_mode": "npz-centered-surface-centered-body-pose",
                "fit_error": float(_mean_bidirectional_nn_distance(template_points, raw_points)),
                "target_npz_name": npz_name,
                "object_id": oid,
                "body_name": body_name,
        ...
        is_flat_stove_burner = (
            canonical_name(category_name) == "flat_stove"
            and cabinet_part == "burner"
        )
        if is_flat_stove_burner:
            legacy_offset = np.asarray(
                (info or {}).get("pose_offset_local", raw_points.mean(axis=0, dtype=np.float32)),
                dtype=np.float32,
            ).reshape(3)
            return {
                "trans_local": legacy_offset.astype(np.float32, copy=False),
                "rot_wxyz": np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32),
                "mapping_mode": "npz-centered-preserve-latent-offset",
                "fit_error": float(_mean_bidirectional_nn_distance(template_points, raw_points)),
                "target_npz_name": npz_name,
                "object_id": oid,
                "body_name": body_name,
        ...
        target_center = target_center_for_oid(oid) if oid >= 0 else None
        if match_kind.startswith("combined") and target_center is not None:
            init_rot = np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32).reshape(4)
            if canonical_name(category_name) == "robot_gripper" or str(entity_name).startswith("robot_gripper__"):
                init_rot = np.asarray(gripper_pose_quat_offset_wxyz(part_name, body_name), dtype=np.float32).reshape(4)

            target_center = np.asarray(target_center, dtype=np.float32).reshape(3)
            asset_path = str((meta or {}).get("asset_path", "") or "").replace("\\", "/").lower()
            source_item = _choose_npz_source_item(spec, meta or {})
            if asset_path.endswith("/microwave_combined_p_scaled.obj"):
                trans_local = target_center.astype(np.float32, copy=False)
                rot_wxyz = init_rot
                mode = "npz-centered-combined-target-center"
            elif asset_path.endswith("/microwave_door_combined_p_scaled.obj"):
                trans_local = raw_points.mean(axis=0, dtype=np.float32).astype(np.float32, copy=False)
                rot_wxyz = init_rot
                mode = "npz-centered-combined-door-raw-center"
            elif source_item is not None:
                item_pos = np.asarray(source_item.get("pos", [0.0, 0.0, 0.0]), dtype=np.float32).reshape(3)
                item_quat = np.asarray(source_item.get("quat", [1.0, 0.0, 0.0, 0.0]), dtype=np.float32).reshape(4)
                trans_local = transform_point_cloud(target_center.reshape(1, 3), item_pos, item_quat)[0].astype(np.float32, copy=False)
                rot_wxyz = quat_mul_wxyz(item_quat, init_rot).astype(np.float32)
                mode = "npz-centered-combined-anchor-item"
            else:
                trans_local = target_center.astype(np.float32, copy=False)
                rot_wxyz = init_rot
                mode = "npz-centered-combined-fallback-target-center"
            return {
        ...
        item = _choose_npz_source_item(spec, meta or {})
        init_rot = np.asarray(item.get("quat", [1.0, 0.0, 0.0, 0.0]), dtype=np.float32).reshape(4) if item is not None else np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32)
        if canonical_name(category_name) == "robot_gripper" or str(entity_name).startswith("robot_gripper__"):
            init_rot = np.asarray(gripper_pose_quat_offset_wxyz(part_name, body_name), dtype=np.float32).reshape(4)
        rot_wxyz, _ = estimate_latent_pose_rotation(
            raw_points,
            template_points,
            init_rot_wxyz=init_rot,
        )
        rot_wxyz = np.asarray(rot_wxyz, dtype=np.float32).reshape(4)
        rot_template_mean = transform_point_cloud(
            template_points.mean(axis=0, dtype=np.float32).reshape(1, 3),
            [0.0, 0.0, 0.0],
            rot_wxyz,
        )[0].astype(np.float32, copy=False)
        trans_local = (raw_points.mean(axis=0, dtype=np.float32) - rot_template_mean).astype(np.float32, copy=False)
        fitted = transform_point_cloud(template_points, trans_local, rot_wxyz)
        fit_error = float(_mean_bidirectional_nn_distance(fitted, raw_points))
        mode = "npz-centered-best-rigid-to-raw"
        return {
            "trans_local": np.asarray(trans_local, dtype=np.float32).reshape(3),
            "rot_wxyz": np.asarray(rot_wxyz, dtype=np.float32).reshape(4),
            "mapping_mode": mode,
    ...
    for entity_name, spec in entity_specs.items():
        entry = _entry_from_spec(entity_name, spec)
        if entry is not None:
            out[entity_name] = entry
    ...
    if gripper_spec is not None:
        for gspec in list(gripper_spec.get("entities", [])):
            ...
            if not entity_name:
                continue
            entry = _entry_from_spec(entity_name, gspec)
            if entry is not None:
                out[entity_name] = entry
...
def process_hdf5_file(
    ...
    latent_ckpt=None,
    ignore_image=False,
    use_latent_pose_alignment=False,
):
    ...
    if use_latent_pose_alignment:
        print("  Mode: active latent pose alignment enabled")
    else:
        print("  Mode: active latent pose alignment disabled (mapping-only)")
    ...
    try:
        ...
        if latent_mapping:
            ...
            npz_to_body_mapping = build_npz_to_body_mapping(
                entity_specs=entity_specs,
                gripper_spec=gripper_spec,
                latent_mapping=latent_mapping,
                object_meta=object_meta,
            ...
            for entity_name, info in latent_mapping.items():
                info.setdefault("pose_rot_offset_wxyz", np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32))
                info.setdefault("pose_align_error", 0.0)
                info.setdefault("alignment_mode", "mapping-only")
        ...
        with h5py.File(output_path, "a") as f:
            if latent_mapping:
                ...
                ordered_entity_names = sorted(set(latent_mapping.keys()) | set(npz_to_body_mapping.keys()))
                ...
                mapping_group.create_dataset(
                    "pose_source_names",
                    data=np.asarray([
                        str(latent_mapping.get(n, {}).get("pose_source", n)) for n in ordered_entity_names
                    ], dtype=str_dtype),
                )
                mapping_group.create_dataset(
                    "target_npz_names",
                    data=np.asarray([
                        str(latent_mapping.get(n, {}).get("target_npz_name", npz_to_body_mapping.get(n, {}).get("target_npz_name", "")))
                ...
                mapping_group.create_dataset(
                    "target_object_ids",
                    data=np.asarray([
                        int(latent_mapping.get(n, {}).get("object_id", npz_to_body_mapping.get(n, {}).get("object_id", -1)))
                        for n in ordered_entity_names
                    ], dtype=np.int32),
                )
                mapping_group.create_dataset(
                    "pose_offset_local",
                    data=np.stack([
                        np.asarray(latent_mapping.get(n, {}).get("pose_offset_local", np.zeros((3,), dtype=np.float32)), dtype=np.float32).reshape(3)
                        for n in ordered_entity_names
                    ], axis=0).astype(np.float32),
                )
                mapping_group.create_dataset(
                    "npz_to_body_trans_local",
                    data=np.stack([
                        np.asarray(npz_to_body_mapping.get(n, {}).get("trans_local", np.zeros((3,), dtype=np.float32)), dtype=np.float32).reshape(3)
                        for n in ordered_entity_names
                    ], axis=0).astype(np.float32),
                )
                mapping_group.create_dataset(
                    ...
                    data=np.stack([
                        np.asarray(latent_mapping.get(n, {}).get("pose_rot_offset_wxyz", np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32)), dtype=np.float32).reshape(4)
                        for n in ordered_entity_names
                    ], axis=0).astype(np.float32),
                )
                mapping_group.create_dataset(
                    "npz_to_body_rot_wxyz",
                    data=np.stack([
                        np.asarray(npz_to_body_mapping.get(n, {}).get("rot_wxyz", np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32)), dtype=np.float32).reshape(4)
                        for n in ordered_entity_names
                    ], axis=0).astype(np.float32),
                )
                mapping_group.create_dataset(
                    "pose_align_error",
                    data=np.asarray([float(latent_mapping.get(n, {}).get("pose_align_error", 0.0)) for n in ordered_entity_names], dtype=np.float32),
                )
                mapping_group.create_dataset(
                    "alignment_mode",
                    data=np.asarray([
                        str(latent_mapping.get(n, {}).get("alignment_mode", "")) for n in ordered_entity_names
                    ], dtype=str_dtype),
                )
                mapping_group.create_dataset(
                    "npz_mapping_mode",
                    data=np.asarray([
                        str(npz_to_body_mapping.get(n, {}).get("mapping_mode", "")) for n in ordered_entity_names
                    ], dtype=str_dtype),
                )
                for field_name, map_key in [
                    ...
                    mapping_group.create_dataset(
                        field_name,
                        data=np.asarray([
                            str(latent_mapping.get(n, {}).get(map_key, npz_to_body_mapping.get(n, {}).get(map_key, "")))
                            for n in ordered_entity_names
                        ], dtype=str_dtype),
                    )
                mapping_group.attrs["latent_ckpt"] = str(latent_ckpt)
                mapping_group.attrs["mapping_version"] = 5
                mapping_group.attrs["npz_template_frame"] = "vnvae_surface_points_centered"
            ...
            for ep in demo_keys:
                ...
                object_poses_npz = {entity_name: [] for entity_name in entity_specs}
                ...
                for t in range(num_steps):
                    ...
                    for entity_name, spec in entity_specs.items():
                        ...
                        elif entity_name in latent_mapping:
                            align_t = np.asarray(
                                latent_mapping[entity_name].get("pose_offset_local", np.zeros((3,), dtype=np.float32)),
                                dtype=np.float32,
                            ).reshape(3)
                            align_q = np.asarray(
                                latent_mapping[entity_name].get("pose_rot_offset_wxyz", np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32)),
                                dtype=np.float32,
                            ).reshape(4)
                        else:
                        ...
                        npz_align = npz_to_body_mapping.get(entity_name, None)
                        if npz_align is not None:
                            npz_t = np.asarray(npz_align.get("trans_local", np.zeros((3,), dtype=np.float32)), dtype=np.float32).reshape(3)
                            npz_q = np.asarray(npz_align.get("rot_wxyz", np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32)), dtype=np.float32).reshape(4)
                            pose_pos_npz = transform_point_cloud(npz_t.reshape(1, 3), pos, quat_raw)[0].astype(np.float32, copy=False)
                            pose_quat_npz = quat_mul_wxyz(quat_raw, npz_q).astype(np.float32)
                        else:
                            pose_pos_npz = pose_pos_aligned
                            pose_quat_npz = pose_quat_aligned
                        object_poses_npz[entity_name].append((pose_pos_npz, pose_quat_npz))
                    ...
                    if has_gripper_entity:
                        for gspec in gripper_entities:
                            ...
                            if align_info is not None:
                                ...
                                align_q = np.asarray(
                                    latent_mapping[entity_name].get("pose_rot_offset_wxyz", q_off),
                                    dtype=np.float32,
                                ).reshape(4)
                            else:
                ...
                npz_pose_group = demo_group.require_group("object_poses_npz")
                for key in list(npz_pose_group.keys()):
                    del npz_pose_group[key]
                for entity_name, pose_list in object_poses_npz.items():
                    pose_arr = np.asarray([np.concatenate([p, q], axis=0) for p, q in pose_list], dtype=np.float32)
                    npz_pose_group.create_dataset(entity_name, data=pose_arr, compression="gzip")
...
def process_dataset(
    ...
    if os.path.isfile(dataset_path):
        ...
        process_hdf5_file(
            ...
            use_latent_pose_alignment=use_latent_pose_alignment,
        )
        return
...
def main():
    ...
    parser.add_argument(
        "--enable-latent-pose-alignment",
        action="store_true",
        help=(
            "Enable active latent-frame pose alignment (including estimated rotation refinement). "
            "Default is off for more stable, deterministic poses on symmetric objects."
        ),
    )
    ...
    process_dataset(
        ...
        use_latent_pose_alignment=bool(args.enable_latent_pose_alignment),
    )
